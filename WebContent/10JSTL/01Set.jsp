...
<body>
	...
	<h3>EL로 출력</h3>
	<!-- 
		EL로 영역의 속성을 읽어 올 때는 XXXScope를 생략 하면 가장 좁은
	...
	<h3>인자생성자로 설정</h3>
	<!-- 인자생성자를 통해 DTO객체를 생성한 후 request영역에 속성(attribute)을 지정한다 -->
	<c:set var="argsMember"
		value='<%=new MemberDTO("KIM","1111","김태희",null) %>'
		scope="request" />
		
	<!-- 영역에 저장 될 때는 Object타입으로 자동형변환 되므로 사용시
	해당 자료형으로 변환(다운캐스텡)해야 한다.ㄴ -->
	<h2>JSP코드로 출력</h2>
	<ul>
		<li>아이디
	...
	<c:set target="${list[0] }" property="name" value="빵딴" />
	
	<h4>target 및 property속성으로 설정후출력</h4>
	<!--  BTS 정보가 출력됨. -->
	<ul>
		<li>아이디:${requestScope.list[0].id }</li>
