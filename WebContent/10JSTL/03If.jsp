...
<!-- 
 ...
 -->
	...
	<!-- 
		...
		true이므로 if블록의 문종이 출력된다. 그리고 result에
		true가 저장된다.
	 -->
	...
	<c:if test="${numVar mod 2 eq 0 }" var="result">
		...
		<h3>위 이프문을 EL식의 삼항식으로 표현</h3>
		${numVar } : ${numvar mod 2 == 0 ? "는 짝수 ":"는 홀수" }
		
		<!-- 
			JSTL의 if문은 else가 없기 때문에 참 or 거짓ㅢ 형태로 만들 때는
		...
		<c:if test=" ${ JAVA eq JAVA } " var="result">
