...
<c:choose>
	<c:when test="${WHEREIS=='UPDATE' }">
		<c:set var="sucmsg" value="수정성공함  ٩( ᐛ )و " />
		<c:set var="failmsg" value="수정실패함  (ಥ⌣ಥ)" />
		<c:set var="sucurl" 
		value="/DataRoom/DataView?idx=${idx}&nowPage=${nowPage}" />
	</c:when>
	<c:otherwise>
		<c:set var="sucmsg" value="삭제성공함  ٩( ᐛ )و " />
		<c:set var="failmsg" value="삭제실패함  (ಥ⌣ಥ)" />
		<c:set var="sucurl" 
