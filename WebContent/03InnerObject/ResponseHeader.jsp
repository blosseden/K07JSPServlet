...
/*
응답헤더 설정하기
	: 새로운 내용을 DB에 추가했는데도 변경된 내용이 웹브라워ㅈ에
	출력되지 않는다면 이는 캐시에 저장된 내용을 출력하고 있기 때문이다.
	아래설정은 캐리ㅅ를 사용하지 않고 매번 새로고침(F5)한 것 처럼
...
파일다운로드 구현시 사용
	...
	다운로드를 위해서는 위와 같은 설정이 필요하다.
*/
//response.setContentType("binary/octect-stream");

/*
응답헤더 추가/설정하기
	add계열 : 기존의 헤더명이 존재할경우 기존의 갑을 유지하면서
...
response.addIntHeader("numberOfCrew", 100);//add newn header
response.setIntHeader("numberOfCrew", 99);//기존 헤더 덛어쓰기(수정) ->99 '만' 출력
%>
<!DOCTYPE html>
<html>
<head>
