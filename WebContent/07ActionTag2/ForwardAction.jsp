...
1. sendRedirect()로 페이지 이동
	-새로운 페이지에 대한 요청이므로 page, request영역 모두 공유되지않는다.
...
*/
	...
	/* response.sendRedirect(request.getContextPath()+
		...
		2.포워드 (자바코드 사용)
			...
			-URL창에는 최초 요청한 페이지의 경로가 보여지므로 사용자는 페이지 이동을 알수 없다.
			-절대 경로로 지정할 경우 Context 루트를 제외한 경로로 지정한다.
		*/
...
<!--
3.액션태그를 통한 포워드
	-액션 태그에 page속성만 지정하면 되므로 표현이 간결하다.
	-나머지 특성은 모두 동일하다.
  -->
