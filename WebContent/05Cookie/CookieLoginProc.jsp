...
/*
	...
	통해 배열로 받아와야 하지만, 항목이 하나만 있는 경우에는 아래와 같이
	처리가능하다.
*/
String id_save= request.getParameter("id_save");

//JDBC연동없이 kosmo/1234일 때 로그인 성공
if("kosmo".equals(userid) && "1234".equals(userpw)){
	...
	if(id_save==null){
		...
		ck.setMaxAge(0);//유효시간이 0이므로 사용 할 수 없는 쿠키가 된다.
		response.addCookie(ck);
	}
	else{
		//체크했을 때
		//사욧ㅇ자가 입력한 아이디로 쿠키를 생성한다
	...
	request.setAttribute("ERROR_MSG","회원아니심");
	//로그인 페이지로 포워드(전달) 한다.
	request.getRequestDispatcher("CookieLoginMain.jsp")
		.forward(request,response);
	//포워드한 페이지는 리퀘스트영역이 공유되므로 위에 저장한 속성을 읽어올 수 이따
}
%>