...
따라서 NullPointerException이 발생하지않는다.
*/
...
만약 
   해당 페이지? myNumber = 20 //인 경우 결과 30 출력됨
   해당페이지? myNumber =    //인 경우 0으로 간주되어 10 출력됨
   해당페이지? myNumber = 삼 //인 경우 문자는 숫자로 변경 불가하므로 에러발생됨

주소창에서 
jsp?myNumber=20 으로 바꿔줄수 있다.
바꿔준후 새로고침하면 아래식의 결과는 10->30으로 변경된다.
-->
...
값은 출력되지않고 null로 인식하게 된다.
JSP의 변수를 EL에서 사용할 수 없는 이유는 EL은 4가지 영역에 저장된
속성들만 사용하기 때문이다.
JSTL도 동일한 특성을 가지고 있다.
*/
String varSciptLet = "스크립트렛 안에서 변수선언";
%>
<!-- null값으로 인식되므로 0으로 간주된다. 결과는 100 출력 -->
\${varSciptLet +100 } : ${varSciptLet + 100 } <br />

<!--  
JSP코드에서 선언한 변수를 EL에서 사용해야할 경우에는 JSTL의 
set태그를 이용해서 변수를 선언한다.
JSP에서 즉시 선언하려면 영역에 저장해야한다.

varSciptLet는 가장 좁은 역역인 페이지 영역에 저장됐다.
${elVar }만 쓸수 있는 이유: 페이지영역이 가장 좁은영역이므로 page생략가능
-->

<c:set var="elVar" value="<%=varSciptLet %>"/>
...
<!-- EL은 표현식에서 자주 쓰이고 할당은 보통 쓰이지않음 

   Tomcat8.0부터는 EL에서 변수할당이 가능해졌다. 그러나 개발시에는
      실제 서비스할 웹서버의 버전을 확인한후 사용여부를 결정한다.
...
<ul>
   ...
   <li>\${fnum / snum } : ${fnum / snum }</li>
   <li>\${fnum div snum } : ${fnuㄴm div snum }</li>
...
\${10 gt 9 ? "참이다 " : "거짓이다"} : 
   ...
   <!--  EL의 empty 연산자 :
      null이거나 ""(빈문자열)일때
      혹은 배열인 경우 길이가 0일때 
      혹은 컬렉션 인경우 size가 0일때 true를 반환하는 연산자 이다.
   -->
   ...
   <ul>
      <li>\${empty elnullStr} : ${empty elnullStr }</li><!--결과 : true  -->
      <li>\${not empty elemptyStr} : ${not empty emptyStr }</li><!-- 결과 : false -->
